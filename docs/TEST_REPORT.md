...
| Scene restart loop (3x) | Playwright Headless Browser | Clean restart without duplicated event listeners | 0 errors across 3 restarts, initial balloon count after restart: 0, listener counts after restart 3: 1/1/1/1 | BROWSER AUTOMATED PASS | Restart loop 3x automated in Playwright script |
...
| Balloon collision owner exit | Playwright Headless Browser | Owner exits, cannot re-enter, opponent blocked | `passThroughPlayerIds` deleted on exit via overlap check, owner re-entry: blocked at x=value, balloon edge=value | BROWSER AUTOMATED PASS | Collision state assertions in `verify-browser.js` |
...
| Production preview game loop | Playwright Headless Browser | Vite preview (`vite preview`) runs playable game | Game loop executable on preview server | BROWSER AUTOMATED PASS | Executed on preview port 4179 via Playwright, second consecutive npm run qa: PASS |
